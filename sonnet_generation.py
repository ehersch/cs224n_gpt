...
def _load_sonnets_from_file(file_path):
  """Load sonnet strings from a file. Supports standard (\\n\\d+\\n) and synthetic (###N###) formats."""
...
def train(args):
  ...
  for epoch in range(args.epochs):
    ...
    if epoch % 5 == 0 and epoch != 0:
      model.eval()
      dev_loss = compute_dev_ce_loss(model, device, args.dev_gold_path, batch_size=args.batch_size)
      ...
      if dev_loss < best_dev_loss - DEV_LOSS_IMPROVEMENT_TOLERANCE:
        ...
        print(f"  New best dev CE loss {best_dev_loss:.3f}, saved as final checkpoint.")
      model.train()

    if best_epoch_dev >= 0 and epoch - best_epoch_dev >= args.early_stopping_patience:
      print(f"Early stopping at epoch {epoch} (no dev loss improvement for {args.early_stopping_patience} epochs).")
      break
...
if modal is not None:
    ...
    for epoch in range(args.epochs):
      ...
      if epoch % 5 == 0 and epoch != 0:
        ...
        if dev_loss < best_dev_loss - DEV_LOSS_IMPROVEMENT_TOLERANCE:
          ...
          print(f"  New best dev CE loss {best_dev_loss:.3f}, saved as only checkpoint.")
        model.train()
  ...
  def train_sft_combined(
    ...
    print(f"Combined dataset: {len(combined_sonnets)} sonnets from {sonnet_path} + {synthetic_sonnet_path}")
    ...
    if args.use_lora:
      ...
      optimizer = AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    params = [p for p in model.parameters() if p.requires_grad] if args.use_lora else model.parameters()
